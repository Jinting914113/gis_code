...
date: "2023-12-08"
---
#Spatial joining-st_join()
in this case the borough data is the left dataset and all the right data has been appended to it
```{r}
library(sf)
...
```
# Static map（静态地图）
## to get OSM, Londonborough, Worldcities, UK_outline
```{r}
##Load all our data
library(sf)
library(tmap)
...
# read in all the spatial data and reproject it(we did before and got OSM)
...
```
## count the Hotels and Airbnb in borough separately
```{r}
# read in the .csv
...
# make a function for the join functions are covered in practical 7
...
Hotels1 <- Joinfun(OSM, Londonborough)
...
UK_outlinecrop <- UK_outline$geometry %>%
  # use the newbb to crop UK_outline$geometry
  st_crop(., newbb)
```
to compare different Airbnb data and Hotels data
```{r}
Hotels <- Hotels1 %>%
...
# 或者：pal1 <-colorBin(palette = "YlOrRd", domain=Joined$`Airbnb count`, bins=breaks)
